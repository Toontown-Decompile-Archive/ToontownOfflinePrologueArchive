...
from toontown.electionsuit.DistributedSuitBaseAI import DistributedSuitBaseAI
...
if simbase.config.GetBool('want-doomsday-reborn', False):
...
class DistributedInvasionSuitAI(DistributedSuitBaseAI, InvasionSuitBase, FSM):
    ...
    def __flyDownComplete(self, task):
        if self.invasion.state == 'Finale':
            if self.spawnPointId == 101:
                self.b_setState('FinaleLockdown')
            else:
    ...
    def attack(self, who):
        attacks = [
         'clip-on-tie', 'redtape', 'newspaper', 'pink-slip', 
         'power-tie']
    ...
    def takeDamage(self, hp, bypassFinale=False):
        ...
        if self.finale or self.spawnPointId == 99:
            if not bypassFinale:
                self.b_setHP(self.currHP + hp)
            return
    ...
    def d_makeHolocog(self):
        self.sendUpdate('makeHolocog', [])
    ...
    def enterFinaleMarch(self, task):
        ...
        if self.finaleX - 1.0 <= oldX <= self.finaleX + 1.0 and self.finaleY - 1.0 <= oldY <= self.finaleY + 1.0 and self.finaleX - 1.0 <= finalX <= self.finaleX + 1.0:
            if self.finaleY - 1.0 <= finalY <= self.finaleY + 1.0:
    ...
    def lockdown(self, task):
        for hood in self.air.hoods:
            if hood.HOOD == ToontownGlobals.ToontownCentral:
                hood.lockdown()

        return task.done

    def enterFinaleLockdown(self):
        self.__lockdownDelay = taskMgr.doMethodLater(20.0, self.lockdown, self.uniqueName('LockdownTTC'))
