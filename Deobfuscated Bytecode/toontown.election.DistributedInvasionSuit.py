...
if base.config.GetBool('want-doomsday-reborn', False):
    import toontown.reborn.SafezoneInvasionRebornGlobals as SafezoneInvasionGlobals
else:
...
class DistributedInvasionSuit(DistributedSuitBase, InvasionSuitBase, FSM, DelayDeletable):
    ...
    def generateAnimDict(self):
        ...
        if self.style.name == 'le':
            animDict['glower'] = 'phase_5/models/char/suitA-glower'
    ...
    def enterFlyDown(self, time):
        ...
        elif self.spawnPointId == 100 or self.spawnPointId == 101:
    ...
    def makeAttackTrack(self):
        if self.style.name == 'le' and random.random() < 0.5 and base.config.GetBool('want-doomsday-reborn', False):
            return self.makeEagleTrack()
        if self.style.name == 'sd' and random.random() < 0.5 and base.config.GetBool('want-doomsday-reborn', False):
            return self.makeDoctorTrack()
        ...
        track = Sequence(Parallel(ActorInterval(self, animName), Track((
    ...
    def makeEagleTrack(self):
        prop = BattleProps.globalPropPool.getProp('evil-eye')
        propIsActor = False
        animName = 'glower'
        x, y, z, h, p, r = (-0.64, 4.45, 5.91, -155.0, -20.0, 0.0)
        prop.setScale(0)
        colNode = CollisionNode('SuitAttack')
        colNode.setTag('damage', str(self.attackDamage))
        ...
        track = Sequence(ActorInterval(self, animName, startFrame=0, endFrame=30), Track((
         0.0, Func(prop.reparentTo, self)), (
         0.0, Func(base.playSfx, sfxEye, volume=0.9, node=self)), (
         0.1, Func(prop.setPosHpr, x, y, z, h, p, r)), (
         0.0, Func(self.sayFaceoffTaunt)), (
         0.1, prop.scaleInterval(1, (11, 11, 11))), (
         0.77, prop.hprInterval(0.02, Point3(205, 40, 0))), (
         throwDelay, Func(throwProp))), ActorInterval(self, animName, startFrame=30, endFrame=0))
        track.prop = prop
        track.propIsActor = propIsActor
        return track

    def makeDoctorTrack(self):
        attackJump = Sequence(ActorInterval(self, 'jump', startFrame=0, endFrame=18), Func(base.playSfx, self.quakeLiftSfx), ActorInterval(self, 'jump', startFrame=18, endFrame=20), ActorInterval(self, 'jump', startFrame=97, endFrame=111), Func(base.playSfx, self.quakeLandSfx), ActorInterval(self, 'jump', startFrame=112, endFrame=138))
        stompSequence = Sequence(Func(self.sayFaceoffTaunt), Wait(1.25), Func(attackJump.start), Wait(1.5), Func(self.applyShakeAttack, base.localAvatar, SafezoneInvasionGlobals.FinaleSuitAttackDamage))
        return stompSequence
    ...
    def enterFinaleLockdown(self, offset):
        self.phraseSequence = Sequence(Func(self.sayFaceoffTaunt, True, SafezoneInvasionGlobals.FinaleSuitPhrases[0]), Wait(7), Func(self.sayFaceoffTaunt, True, SafezoneInvasionGlobals.FinaleSuitPhrases[1]), Wait(7), Func(self.sayFaceoffTaunt, True, SafezoneInvasionGlobals.FinaleSuitPhrases[2]), Wait(7), Func(self.applyShakeAttack, base.localAvatar, ToontownGlobals.MaxHpLimit))
        self.phraseSequence.setT(offset)
        self.phraseSequence.start()

    def exitFinaleLockdown(self):
        pass
