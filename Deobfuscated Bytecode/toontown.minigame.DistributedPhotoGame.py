...
class DistributedPhotoGame(DistributedMinigame, PhotoGameBase.PhotoGameBase):
    ...
    def load(self):
        ...
        self.filmPanel = DirectLabel(parent=hidden, relief=None, pos=(-0.23, -1.2,
                                                                      -0.55), scale=0.65, text=str(self.filmCount), text_scale=0.2, text_fg=(0.95,
                                                                                                                                             0.95,
                                                                                                                                             0,
                                                                                                                                             1), text_pos=(0.08,
                                                                                                                                                           -0.15), text_font=ToontownGlobals.getSignFont(), image=self.filmImage, image_scale=Point3(1.0, 0.0, 0.85))
        self.filmPanelTitle = DirectLabel(parent=self.filmPanel, relief=None, pos=(0.08,
                                                                                   0,
                                                                                   0.04), scale=0.08, text=TTLocalizer.PhotoGameFilm, text_fg=(0.95,
                                                                                                                                               0.95,
                                                                                                                                               0,
                                                                                                                                               1), text_shadow=(0,
                                                                                                                                                                0,
    ...
    def makeAssignmentPanel(self, assignment):
        ...
        self.starDict[model] = []
        for index in range(NUMSTARS):
            star = DirectLabel(parent=starParent, image=starImage, image_color=(1,
                                                                                1,
                                                                                1,
                                                                                1), image_scale=Point3(STARSIZE, 0.0, STARSIZE), relief=None)
    ...
    def makeAssignmentFrame(self):
        ...
        frame = DirectFrame(parent=hidden, image=photoImage, image_color=(1, 1, 1,
                                                                          1), image_scale=Point3(1.6 * assignmentScale, 0.0, 1.75 * assignmentScale), frameSize=(-size,
         size,
         -size,
    ...
    def makeScoreFrame(self):
        ...
        frame = DirectFrame(parent=hidden, image=scoreImage, image_color=(1, 1, 1,
                                                                          1), image_scale=Point3(0.64, 0.0, 0.64), frameSize=(-size,
         size,
         -size,
    ...
    def __localPhotoMoveTask(self, task):
        if not hasattr(self, 'swivel'):
            return
        pos = [
         self.swivel.getHpr()[0], self.swivel.getHpr()[1], self.swivel.getHpr()[2]]
        oldRot = pos[0]
        oldAng = pos[1]
        rotVel = 0
        if self.leftPressed:
