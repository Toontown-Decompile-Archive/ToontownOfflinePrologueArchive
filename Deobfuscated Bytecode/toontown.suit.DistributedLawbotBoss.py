...
class DistributedLawbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def loadScale(self):
        self.useProgrammerScale = config.GetBool('want-injustice-scale-debug', 0)
    ...
    def makeIntroductionMovie(self, delayDeletes):
        ...
        dialogTrack = Track((
         ...
         24,
         Sequence(Func(self.clearChat), self.loseCogSuits(self.toonsA + self.toonsB, render, (-2.798,
                                                                                     -70,
                                                                                     10,
                                                                                     180,
                                                                                     0,
                                                                                     0)))), (
         27,
         Sequence(self.toonNormalEyes(self.involvedToons), Func(self.loop, 'Ff_neutral'), Func(self.setChatAbsolute, attackToons, CFSpeech))))
    ...
    def makeVictoryMovie(self):
        ...
        bossTrack = Track((
         0.5,
         Sequence(Func(self.clearChat), Func(base.camera.reparentTo, render), Func(base.camera.setPos, -3, 45, 25), Func(base.camera.setHpr, 0, 10, 0))), (
         ...
         9.6,
         Parallel(rollTrack, Func(self.setChatAbsolute, TTLocalizer.LawbotBossDefenseWins3, CFSpeech), self.door3.posInterval(2, doorEndPos, startPos=doorStartPos))), (
    ...
    def calculateWeightOfToon(self, toonId):
        ...
        return (
         newWeight, bonusWeight, numJurors)