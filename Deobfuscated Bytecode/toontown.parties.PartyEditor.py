...
class PartyEditor(DirectObject, FSM):
    ...
    def __init__(self, partyPlanner, parent):
        ...
        self.defaultTransitions = {'Hidden': ['Idle', 'Cleanup'], 'Idle': [
                  'DraggingElement', 'Hidden', 'Cleanup'], 
           'DraggingElement': [
                             'Idle',
                             'DraggingElement',
                             'Hidden',
    ...
    def initElementList(self):
        ...
        self.elementList = DirectScrolledList(parent=self.parent, relief=None, decButton_image=(self.partyPlanner.gui.find('**/activitiesButtonUp_up'),
         ...
         self.partyPlanner.gui.find('**/activitiesButtonUp_inactive')), decButton_relief=None, decButton_pos=(-0.05,
                                                                                                              0.0,
                                                                                                              -0.38), incButton_image=(self.partyPlanner.gui.find('**/activitiesButtonDown_up'),
         self.partyPlanner.gui.find('**/activitiesButtonDown_down'),
         self.partyPlanner.gui.find('**/activitiesButtonDown_rollover'),
         self.partyPlanner.gui.find('**/activitiesButtonDown_inactive')), incButton_relief=None, incButton_pos=(-0.05,
                                                                                                                0.0,
                                                                                                                -0.94), itemFrame_pos=(pos[0], pos[1], pos[2] + 0.04), itemFrame_relief=None, numItemsVisible=1, items=[])
    ...
    def handleMutuallyExclusiveActivities(self):
        ...
        for act in currentActivities:
            if act.id in mutSet and not lastActivity == act.id:
                ...
                instr = TTLocalizer.PartyPlannerEditorInstructionsRemoved % {'removed': removedName, 'added': addedName}
