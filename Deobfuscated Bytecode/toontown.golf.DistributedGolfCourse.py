...
class DistributedGolfCourse(DistributedObject.DistributedObject, FSM, DelayDeletable):
    ...
    defaultTransitions = {'Off': ['Join'], 'Join': [
              'WaitStartHole', 'Cleanup'], 
       'WaitStartHole': [
                       'PlayHole', 'Cleanup', 'WaitReward'], 
       'PlayHole': [
                  'WaitFinishCourse',
                  'WaitStartHole',
                  'WaitReward',
    ...
    def __init__(self, cr):
        ...
        self.waitingStartLabel = DirectLabel(text=TTLocalizer.MinigameWaitingForOtherPlayers, text_fg=VBase4(1, 1, 1, 1), relief=None, pos=(-0.6,
                                                                                                                                            0,
                                                                                                                                            -0.75), scale=0.075)
        ...
        if self.toonPanels and self.localAvId != avId:
            ...
            for x in xrange(len(self.avIdList)):
                if avId == self.avIdList[x] and y < len(self.toonPanels):
                    ...
                    toonPanel.tag1.hide()
                    toonPanel.tag2.hide()
                    exitedToon = DirectLabel(parent=self.toonPanels[y], relief=None, pos=(0,
                                                                                          0,
                                                                                          0.4), color=(1,
                                                                                                       1,
                                                                                                       1,
                                                                                                       1), text_align=TextNode.ACenter, text=TTLocalizer.GolferExited % toonPanel.av.getName(), text_scale=0.07, text_wordwrap=6)
