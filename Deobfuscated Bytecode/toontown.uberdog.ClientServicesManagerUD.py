...
import dumbdbm, time, hmac, hashlib, json
...
REPORT_REASONS = [
 ...
 'MODERATION_BAD_NAME', 'MODERATION_HACKING']
...
class RemoteAccountDB:
    ...
    def lookup(self, cookie, callback):
        ...
        if not response:
            callback({'success': False, 'reason': 'Account server failed to respond properly.'})
        elif not response.get('status') or not response.get('valid'):
            callback({'success': False, 'reason': response.get('banner', 'Failed for unknown reason')})
        else:
            gsUserId = response.get('gs_user_id', -1)
            if gsUserId == -1:
                gsUserId = 0
            callback({'success': True, 'databaseId': response['user_id'], 
               'accountId': gsUserId, 
...
class LoginAccountFSM(OperationFSM):
    ...
    def __handleLookup(self, result):
        ...
        if self.adminAccess < simbase.config.GetInt('minimum-access', 100):
            ...
            self.demand('Kill', result.get('reason', 'You have insufficient access to login.\nYou have access level %d and you need access level %d.' % (self.adminAccess, simbase.config.GetInt('minimum-access', 100))))
            return
        if accountId:
...
class CreateAvatarFSM(OperationFSM):
    ...
    def __handleRetrieve(self, dclass, fields):
        ...
        if self.avList[self.index]:
            self.demand('Kill', 'Avatar slot %d is already taken by another avatar (%s)!' % (self.index, str(self.avList[self.index])))
            return
...
class GetAvatarsFSM(AvatarOperationFSM):
    ...
    def enterSendAvatars(self):
        ...
        self.csm.sendUpdateToAccountId(self.target, 'setTrialer', [self.trialer])
...
class SetNameTypedFSM(AvatarOperationFSM):
    ...
    def __handleAvatar(self, dclass, fields):
        ...
        if fields['WishNameState'] != WISHNAME_OPEN:
            self.demand('Kill', 'Avatar %d is not supposed to be able to be named right now! Its name status is %d.' % (self.avId, fields['WishNameState']))
            return
        self.demand('JudgeName')
    ...
    def enterJudgeName(self):
        ...
        if self.avId and status:
            self.csm.air.dbInterface.updateObject(self.csm.air.dbId, self.avId, self.csm.air.dclassesByName['DistributedToonUD'], {'WishNameState': WISHNAME_APPROVED, 'WishName': self.name, 
