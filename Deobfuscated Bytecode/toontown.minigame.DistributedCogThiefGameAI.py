...
class DistributedCogThiefGameAI(DistributedMinigameAI.DistributedMinigameAI):
    ...
    def initCogInfo(self):
        for cogIndex in xrange(self.getNumCogs()):
            self.cogInfo[cogIndex] = {'pos': Point3(CogThiefGameGlobals.CogStartingPositions[cogIndex]), 'goal': CTGG.NoGoal, 
    ...
    def initBarrelInfo(self):
        for barrelIndex in xrange(CogThiefGameGlobals.NumBarrels):
            self.barrelInfo[barrelIndex] = {'pos': Point3(CogThiefGameGlobals.BarrelStartingPositions[barrelIndex]), 'carriedBy': CTGG.BarrelOnGround, 'stolen': False}
