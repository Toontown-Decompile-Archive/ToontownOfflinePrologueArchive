...
class DistributedCogKart(DistributedElevatorExt.DistributedElevatorExt):
    ...
    def kartDoorsClosed(self, zoneId):
        if self.localToonOnBoard:
            ...
            doneStatus = {'loader': 'suitInterior', 'where': 'suitInterior', 
               'hoodId': hoodId, 
               'zoneId': zoneId, 
               'shardId': None}
    ...
    def setCountryClubInteriorZone(self, zoneId):
        if self.localToonOnBoard:
            ...
            doneStatus = {'loader': 'cogHQLoader', 'where': 'countryClubInterior', 'how': 'teleportIn', 
               'zoneId': zoneId, 
               'countryClubId': self.countryClubId, 
               'hoodId': hoodId}
    ...
    def rejectBoard(self, avId, reason=0):
        ...
        if hasattr(base.localAvatar, 'elevatorNotifier'):
            if reason == ElevatorConstants.REJECT_SHUFFLE:
                ...
                if reason == ElevatorConstants.REJECT_MINLAFF:
