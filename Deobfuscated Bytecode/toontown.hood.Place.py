...
import TrialerForceAcknowledge, ZoneUtil
...
class Place(StateData.StateData, FriendsListManager.FriendsListManager):
    ...
    def handleBookCloseTeleport(self, hoodId, zoneId):
        if localAvatar.hasActiveBoardingGroup():
            ...
            self.requestLeave({'loader': ZoneUtil.getBranchLoaderName(zoneId), 'where': ZoneUtil.getToonWhereName(zoneId), 
               'how': 'teleportIn', 
               'hoodId': hoodId, 
               'zoneId': zoneId, 
               'shardId': None, 
    ...
    def goHomeNow(self, curZoneId):
        if localAvatar.hasActiveBoardingGroup():
            ...
            self.requestLeave({'loader': 'safeZoneLoader', 'where': 'estate', 
               'how': 'teleportIn', 
               'hoodId': hoodId, 
               'zoneId': -1, 
               'shardId': None, 
    ...
    def handleEnterTunnel(self, requestStatus, collEntry):
        if localAvatar.hasActiveBoardingGroup():
            ...
            del dummyNP
            return
        if requestStatus['zoneId'] == 19000 and base.localAvatar.cogParts[4] != CogDisguiseGlobals.PartsPerSuitBitmasks[4]:
            base.localAvatar.disableAvatarControls()
            rejectText = TTLocalizer.FADoorCodes_HB_DISGUISE_INCOMPLETE
            base.localAvatar.disableAvatarControls()
            self.confirm = TTDialog.TTGlobalDialog(doneEvent='confirmDone', message=rejectText, style=TTDialog.Acknowledge)
            self.confirm.show()
    ...
    def enterDFACallback(self, requestStatus, doneStatus):
        ...
        if doneStatus['mode'] == 'complete':
            if requestStatus.get('tutorial', 0):
                ...
                out = {'teleportIn': 'teleportOut', 'tunnelIn': 'tunnelOut', 
                   'doorIn': 'doorOut'}
    ...
    def enterTunnelInSewer(self, requestStatus):
        self.notify.debug('enterTunnelIn(requestStatus=' + str(requestStatus) + ')')
        base.transitions.irisIn()
        self.accept('tunnelInMovieDone', self.__tunnelInMovieDone)
        base.localAvatar.reconsiderCheesyEffect()
        base.localAvatar.setPosHpr(0, -27, 5.65, 0, 0, 0)
    ...
    def enterTunnelOut(self, requestStatus):
        ...
        self.doneStatus = {'loader': ZoneUtil.getLoaderName(zoneId), 'where': ZoneUtil.getToonWhereName(zoneId), 'how': how, 
           'hoodId': hoodId, 
           'zoneId': zoneId, 
           'shardId': None, 
    ...
    def enterTeleportOut(self, requestStatus, callback):
        base.localAvatar.laffMeter.start()
        if requestStatus['zoneId'] == ToontownGlobals.CogNationCentral:
            base.localAvatar.b_setAnimState('TeleportIn', 1, callback, [requestStatus])
        else:
    ...
    def enterDied(self, requestStatus, callback=None):
        if self.zoneId % 1000 == 0 and base.config.GetBool('want-doomsday-reborn', False):
    ...
    def requestTeleport(self, hoodId, zoneId, shardId, avId):
        ...
        if hoodId == ToontownGlobals.PartyHood:
            loaderId = 'safeZoneLoader'
            whereId = 'party'
        self.requestLeave({'loader': loaderId, 'where': whereId, 
           'how': 'teleportIn', 
           'hoodId': hoodId, 
           'zoneId': zoneId, 
           'shardId': shardId, 
...
def tp(hood):
    global HOOD
    if hasattr(base.cr, 'choseScrooge') and base.cr.choseScrooge:
        return '~tp is deactivated during the Prologue!'
    try:
        HOOD = hood.upper()
