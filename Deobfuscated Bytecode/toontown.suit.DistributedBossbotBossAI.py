...
class DistributedBossbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def recordCeoInfo(self):
        ...
        self.air.writeServerEvent('ceoInfo', doId=self.doId, victory=didTheyWin, battleTIme=self.battleFourTimeInMin, difficulty=self.battleDifficulty, numToonsAtStart=self.numToonsAtStart, numToonsAtEnd=self.numToonsAtEnd, numTables=self.numTables, numDiners=self.numTables * self.numDinersPerTable, dinersKilled=self.numDinersExploded, toonHps=toonHps, involvedToons=self.involvedToons, speedDamage=self.speedDamage, numMoveAttacks=self.numMoveAttacks, numGolfAttacks=self.numGolfAttacks, numGearAttacks=self.numGearAttacks, numGolfAreaAttacks=self.numGolfAreaAttacks, toonupsGranted=self.numToonupGranted, laffHealed=self.totalLaffHealed)
