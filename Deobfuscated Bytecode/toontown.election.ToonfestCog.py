...
class ToonfestCog(FSM):
    ...
    def __init__(self, parentNode, id, bounceSpeed=3, bounceHeight=1, rotateSpeed=1, heightShift=1, xMoveSpeed=0, xMoveDistance=0, bounceOffset=0):
        self.id = id
        FSM.__init__(self, 'ToonfestCogFSM-%d' % self.id)
    ...
    def load(self):
        ...
        self.actor = Actor(path + 'actor', {'idle': path + 'idle_anim', 'down': path + 'down_anim', 
           'up': path + 'up_anim', 
    ...
    def enterActive(self, startTime):
        ...
        taskMgr.add(updateTask, 'ToonfestCog.update-%d' % self.id)

    def exitActive(self):
        taskMgr.remove('ToonfestCog.update-%d' % self.id)
        taskMgr.remove('ToonfestCog.bounceTask-%d' % self.id)
        self.clearHitInterval()
    ...
    def startBounce(self):
        taskMgr.add(self.bounce, 'ToonfestCog.bounceTask-%d' % self.id)
    ...
    def hideHitScore(self):
        if self.hpText:
            taskMgr.remove('ToonfestCogHpText' + str(self.id))
