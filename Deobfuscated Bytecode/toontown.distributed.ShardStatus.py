...
class ShardStatusSender:
    ...
    def update_heap(self):
        ...
        while taskMgr.running:
            if lastUpdate < time.time() - shard_heap_interval.getValue():
                self.heap_status = {'objects': len(gc.get_objects()), 'garbage': len(gc.garbage)}
                lastUpdate = time.time()
            time.sleep(1.0)

    def start(self):
        globalClock.setAverageFrameRateInterval(shard_status_interval.getValue())
        offlineStatus = {'channel': self.air.ourChannel, 'offline': True}
    ...
    def sendStatus(self):
        ...
        status = {'channel': self.air.ourChannel, 'districtId': self.air.distributedDistrict.doId, 
