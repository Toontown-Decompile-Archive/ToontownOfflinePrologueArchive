...
class DistributedPrologue2Event(DistributedObject, FSM):
    ...
    def __cleanupNPCs(self):
        npcs = [
         self.nigger, self.logano, self.sparky, self.weird, self.ned, self.zzandrew, self.jaquanza, self.supertricky, self.patrick]
        for npc in npcs:
            if npc:
                npc.removeActive()
                npc.hide()
