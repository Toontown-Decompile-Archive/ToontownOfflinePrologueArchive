...
class DistributedLawbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def enterBattleThree(self):
        ...
        self.notify.debug('diffLevel=%d ammoCount=%d gavels=%d lawyers = %d, toonup=%d' % (self.battleDifficulty,
         ...
         self.numLawyers,
         self.toonupValue))
        self.air.writeServerEvent('lawbotBossSettings', doId=self.doId, dept=self.dept, difficulty=self.battleDifficulty, ammoCount=self.ammoCount, numGavels=self.numGavels, numLawyers=self.numLawyers, toonupValue=self.toonupValue)
    ...
    def doBattleThreeInfo(self):
        ...
        self.air.writeServerEvent('b3Info', doId=self.doId, victory=didTheyWin, time=self.battleThreeTimeInMin, numToonsAtStart=self.numToonsAtStart, numToonsAtEnd=self.numToonsAtEnd, toonJurors=self.numToonJurorsSeated, difficulty=self.battleDifficulty, ammoCount=self.ammoCount, numGavels=self.numGavels, numLawyers=self.numLawyers, toonupValue=self.toonupValue, numBonuses=self.numBonusStates, numAreaAttacks=self.numAreaAttacks, toonHps=toonHps, weightPerToon=self.weightPerToon)
        ...
        if hasSummon:
            ...
            if not toon.hasParticularCogSummons(prefDeptIndex, cogLevel, 'single'):
                ...
                if not toon.hasParticularCogSummons(prefDeptIndex, cogLevel, 'building'):
                    ...
                    if not toon.hasParticularCogSummons(prefDeptIndex, cogLevel, 'invasion'):
                        summonType = 'invasion'
                    else:
                        foundOne = False
