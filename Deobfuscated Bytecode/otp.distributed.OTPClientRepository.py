...
class OTPClientRepository(ClientRepositoryBase):
    ...
    WishNameResult = Enum(['Failure',
     ...
     'Approved',
     'Rejected'])
    if config.GetBool('want-whitelist', False):
        whiteListChatEnabled = 1
    else:
        whiteListChatEnabled = 0
    ...
    def __init__(self, serverVersion, launcher=None, playGame=None):
        ...
        if config.GetBool('want-membership', True):
            self.__isPaid = 1
        else:
            self.__isPaid = 0
    ...
    @report(types=['args', 'deltaStamp'], dConfigParam='teleport')
    def __handleDropConnection(self):
        self.loginFSM.request('shutdown')
