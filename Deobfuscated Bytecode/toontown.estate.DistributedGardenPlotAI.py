...
class DistributedGardenPlotAI(DistributedLawnDecorAI):
    ...
    def plantFlower(self, species, variety, usingFlowerAll=0):
        av = self.__initialSanityCheck((usingFlowerAll or GardenGlobals).FLOWER_TYPE if 1 else None, usingFlowerAll)
        if not av:
            return
        else:
            ...
            def _plant(task):
                ...
                self.air.writeServerEvent('plant-flower', self.__plantingAvId, species=species, variety=variety, plot=self.plot, name=attr.get('name', 'unknown flower'))
                if task:
                    return task.done
...
def gibSpecials():
    av = spellbook.getTarget()
    av.gardenSpecials = []
    for x in (100, 101, 103, 105, 106, 107, 108, 110, 130, 131, 135):
