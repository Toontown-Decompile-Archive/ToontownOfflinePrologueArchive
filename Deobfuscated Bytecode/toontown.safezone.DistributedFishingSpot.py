...
class DistributedFishingSpot(DistributedObject.DistributedObject):
    ...
    def __handleEnterSphere(self, collEntry):
        if base.config.GetBool('want-doomsday', False) or base.config.GetBool('want-doomsday-reborn', False):
            base.localAvatar.disableAvatarControls()
            self.confirm = TTDialog.TTGlobalDialog(doneEvent='confirmDone', message=SafezoneInvasionGlobals.FishingAlert, style=TTDialog.Acknowledge)
            self.confirm.show()
    ...
    def __loadStuff(self):
        ...
        if self.splashSounds == None:
            self.splashSounds = (
             base.loadSfx('phase_4/audio/sfx/TT_splash1.ogg'), base.loadSfx('phase_4/audio/sfx/TT_splash2.ogg'))
        return
    ...
    def enterReward(self, code, itemDesc1, itemDesc2, itemDesc3):
        ...
        if self.localToonFishing:
            ...
            if code == FishGlobals.QuestItem:
                ...
                if code in (FishGlobals.FishItem, FishGlobals.FishItemNewEntry, FishGlobals.FishItemNewRecord):
            ...
            if code == FishGlobals.JellybeanItem:
                amount = itemDesc1
