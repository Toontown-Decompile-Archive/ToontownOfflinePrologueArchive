...
class CogdoFlyingGame(DirectObject):
    ...
    def handleLocalPlayerTargetedByEagle(self):
        if not self.localPlayer.isInvulnerable() and not self._hints['targettedByEagle']:
            if self.invSuit:
                anOrA = 'An ' if SuitBattleGlobals.SuitAttributes[self.invSuit]['name'][0] in ('A',
                                                                                               'E',
                                                                                               'I',
                                                                                               'O',
                                                                                               'U') else 'A '
                self.guiMgr.setMessage(TTLocalizer.CogdoFlyingGameInvasionTargeting % (anOrA + SuitBattleGlobals.SuitAttributes[self.invSuit]['name']))
            else:
