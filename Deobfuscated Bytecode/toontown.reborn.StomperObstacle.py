...
# Embedded file name: toontown.reborn.StomperObstacle
...
class StomperObstacle(NodePath):
    notify = DirectNotifyGlobal.directNotify.newCategory('StomperObstacle')
    ...
    def __init__(self, parent):
        node = hidden.attachNewNode('StomperObstacle')
        ...
        self.modelPath = 0
        self.style = 'vertical'
        self.headScale = Point3(7, 5, 7)
        self.crushMsg = 'crushMsg-%s' % id(self)
        self.shaftScale = Point3(0.5, 12, 0.5)
        self.motion = 3
        self.startTime = ClockDelta.globalClockDelta.networkToLocalTime(globalClock.getRealTime(), bits=32)
        self.range = 30.0
        self.soundOn = 1
        self.period = 4.0
        self.phaseShift = 0.0
        self.damageList = [5, 10, 12]
        self.loaded = 0
        self.crushedList = []
        self.sounds = []
        self.wantSmoke = 1
    ...
    def loadModel(self):
        ...
        if self.cogStyle == 1:
            self.stomperModels = [
             'phase_11/models/lawbotHQ/LB_square_stomper']
        ...
        if self.style == 'vertical':
            ...
            self.parent.accept(self.crushMsg, self.checkSquashedToon)
    ...
    def startStomper(self, startTime=0, mode=STOMPER_START):
        if self.ival:
            self.ival.pause()
            del self.ival
            self.ival = None
        ...
        if motionIval == None:
            ...
            self.ival = Parallel(Sequence(motionIval, Func(self.__startSmokeTask), Func(self.sendStompToon)), name='Stomper-%s' % id(self))
            if wantSound:
            ...
            if mode == STOMPER_START:
                self.ival.loop()
                self.ival.setT(globalClock.getFrameTime() - self.startTime + self.period * self.phaseShift)
            else:
    ...
    def __startSmokeTask(self):
        taskMgr.remove('smokeTask')
        if self.wantSmoke:
            taskMgr.add(self.__smokeTask, 'smokeTask')
    ...
    def checkSquashedToon(self):
        if self.style == 'vertical':
            ...
            if tPos[2] < zRange and tPos[2] > -zRange and tPos[0] < xRange and tPos[0] > -xRange and tPos[1] < yRange / 10.0 and tPos[1] > -yRange:
                self.parent.b_setOuch(random.choice(self.damageList), 'Squish')
                base.localAvatar.setZ(self.getZ(render) + 0.025)

    def getParentToken(self):
        return 1000000 * id(self)
