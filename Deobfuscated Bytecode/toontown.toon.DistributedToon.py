...
import time, operator, random, copy
if base.wantKarts:
...
class DistributedToon(DistributedPlayer.DistributedPlayer, Toon.Toon, DistributedSmoothNode.DistributedSmoothNode, DelayDeletable):
    ...
    def setDefaultZone(self, zoneId):
        if zoneId >= 20000 and zoneId < 22000:
            zoneId = zoneId + 2000
        if base.localAvatar.getName() == 'Scrooge McDuck' and self.style.getType() == 'scrooge':
            zoneId = 1002000
        try:
            hoodPhase = base.cr.hoodMgr.getPhaseFromHood(zoneId)
        except:
            if base.localAvatar.getName() == 'Scrooge McDuck' and self.style.getType() == 'scrooge':
                self.defaultZone = ToontownGlobals.ScroogeBank
            else:
        ...
        if not base.cr.isPaid() or launcher and not launcher.getPhaseComplete(hoodPhase):
            if base.localAvatar.getName() == 'Scrooge McDuck' and self.style.getType() == 'scrooge':
                ...
                self.defaultZone = ToontownGlobals.ToontownCentral
        elif base.localAvatar.getName() == 'Scrooge McDuck' and self.style.getType() == 'scrooge':
            self.defaultZone = ToontownGlobals.ScroogeBank
        else:
    ...
    def died(self):
        ...
        if self.isLocal():
            ...
            if localAvatar.zoneId == target_sz and target_sz == 2000 and base.config.GetBool('want-doomsday-reborn', False):
                target_sz = ToontownGlobals.Sewer
    ...
    def magicTeleportInitiate(self, hoodId, zoneId):
        ...
        if ZoneUtil.isDynamicZone(zoneId) and hoodId in [ToontownGlobals.BossbotHQ, ToontownGlobals.LawbotHQ, ToontownGlobals.CashbotHQ, ToontownGlobals.SellbotHQ, ToontownGlobals.CogNationCentral]:
            whereId = 'cogHQBossBattle'
        if zoneId in [ToontownGlobals.BossbotLobby, ToontownGlobals.LawbotLobby, ToontownGlobals.CashbotLobby, ToontownGlobals.SellbotLobby, ToontownGlobals.LawbotOfficeExt, ToontownGlobals.CogNationCentralLobby]:
            how = 'walk'
        else:
            how = 'teleportIn'
        requestStatus = [
