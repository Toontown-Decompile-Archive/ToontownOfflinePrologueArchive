...
# Embedded file name: toontown.fnaf.Level
...
class DynamicCogDoor(CogDoor):
    ...
    def __init__(self, model, **kw):
        ...
        cNode.addSolid(CollisionSphere(0, 0, 0, 0.8))
...
class Level:
    ...
    def load(self):
        ...
        self.addQuadrant('03', (67, 238, 0), (180, 0, 0), zScale=0.5)
        self.addStaticDoor(pos=(-111, 90, 10), scale=(0.8, 1, 0.75), state='Open')
        self.addStaticDoor(pos=(-111, 220, 10), scale=(0.8, 1, 0.75), state='Closed')
        self.dynamicDoors = []
        self.addDynamicDoor(pos=(45, 0, 0), hpr=(90, 0, 0), scale=0.65, buttonPos=(0,
                                                                                   20,
                                                                                   0), state='Open')
        self.addDynamicDoor(pos=(-45, 0, 0), hpr=(90, 0, 0), scale=0.65, buttonPos=(0,
                                                                                    -20,
        ...
        self.fog.setExpDensity(0.03)
        self.np.setFog(self.fog)
        self.poster = loader.loadModel('phase_9/models/cogHQ/poster_mickey')
        self.posterpn = loader.loadModel('phase_9/models/cogHQ/poster_mickey_pn')
        self.poster.reparentTo(render)
        self.posterpn.reparentTo(render)
        self.poster.setPosHpr(-80.94, 44.36, 2.5, -90, 0, 0)
        self.poster.setScale(5)
        self.posterpn.setPosHpr(-80.94, 44.36, 2.5, -90, 0, 0)
        self.posterpn.setScale(5)
        self.poster.setColorScale(0.5, 0.5, 0.5, 1)
        self.poster.stash()
        self.posterpn.stash()
        self.bgm = loader.loadMusic(self.getBgm())
        self.bgm.setLoopCount(0)
        self.unnerve = loader.loadMusic('phase_9/audio/bgm/fnaf_unnerve.ogg')
        self.unnerve.setLoop(0)
    ...
    def enter(self, night=1):
        ...
        for cog in self.cogs:
            cog.resetPos()
            if cog.type == 'A':
                quad = 'ControlRoomCamera2'
            elif cog.type == 'B':
                quad = 'SafeRoom'
            elif cog.type == 'C':
                quad = 'ControlRoomCamera1'
    ...
    def unload(self):
        ...
        self.quadrants = set()
        self.poster.removeNode()
        self.posterpn.removeNode()
        self.np.removeNode()
