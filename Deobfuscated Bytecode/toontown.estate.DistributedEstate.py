...
import GardenDropGame, GardenProgressMeter
...
class DistributedEstate(DistributedObject.DistributedObject):
    ...
    def loadWitch(self):
        if not self.airplane:
            self.airplane = loader.loadModel('phase_4/models/props/tt_m_prp_ext_flyingWitch')
    ...
    def getDeltaTime(self):
        ...
        self.notify.debug('getDeltaTime = %s. curTime=%s. dawnTime=%s. serverTime=%s.  deltaTime=%s' % (
