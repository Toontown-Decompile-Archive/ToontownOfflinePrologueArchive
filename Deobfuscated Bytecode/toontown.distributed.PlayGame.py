...
from toontown.hood import ODGHood
from toontown.hood import SBHood
...
from toontown.hood import SEHood
from toontown.hood import SellbotHQ, CashbotHQ, LawbotHQ, BossbotHQ, CogNationCentral
...
class PlayGame(StateData.StateData):
    ...
    Hood2ClassDict = {ToontownGlobals.ToontownCentral: TTHood.TTHood, ToontownGlobals.DonaldsDock: DDHood.DDHood, 
       ...
       ToontownGlobals.CogNationCentral: CogNationCentral.CogNationCentral, 
       ToontownGlobals.Sewer: SEHood.SEHood, 
    ...
    Hood2StateDict = {ToontownGlobals.ToontownCentral: 'TTHood', ToontownGlobals.DonaldsDock: 'DDHood', 
       ...
       ToontownGlobals.CogNationCentral: 'CogNationCentral', 
       ToontownGlobals.Sewer: 'SEHood', 
    ...
    def __init__(self, parentFSM, doneEvent):
        ...
        self.fsm = ClassicFSM.ClassicFSM('PlayGame', [State.State('start', self.enterStart, self.exitStart, ['quietZone']),
         ...
         State.State('ODGHood', self.enterODGHood, self.exitODGHood, ['quietZone']),
         State.State('SBHood', self.enterSBHood, self.exitSBHood, ['quietZone']),
         ...
         State.State('SEHood', self.enterSEHood, self.exitSEHood, ['quietZone']),
         State.State('CogNationCentral', self.enterCogNationCentral, self.exitCogNationCentral, ['quietZone']),
    ...
    def enter(self, hoodId, zoneId, avId):
        ...
        self.fsm.request('quietZone', [
         {'loader': loaderName, 'where': whereName, 
            'how': 'teleportIn', 
            'hoodId': hoodId, 
            'zoneId': zoneId, 
            'shardId': None, 
    ...
    def loadDnaStoreTutorial(self):
        ...
        loader.loadDNAFile(self.dnaStore, 'phase_3.5/dna/storage_interior.pdna')
    ...
    def handleWaitForSetZoneResponse(self, requestStatus):
        ...
        if not loader.inBulkBlock:
            if hoodId == ToontownGlobals.MyEstate:
                ...
                loader.beginBulkLoad('hood', TTLocalizer.HeadingToHood % {'to': toHoodPhrase, 'hood': hoodName}, count, 1, TTLocalizer.TIP_COGHQ)
            elif zoneId == ToontownGlobals.ScroogeBank:
                loader.beginBulkLoad('hood', TTLocalizer.HeadingToHood % {'to': TTLocalizer.Prologue[0], 'hood': TTLocalizer.Prologue[2]}, count, 1, TTLocalizer.TIP_GENERAL)
            else:
                if ZoneUtil.isGoofySpeedwayZone(zoneId):
                    loader.beginBulkLoad('hood', TTLocalizer.HeadingToHood % {'to': toHoodPhrase, 'hood': hoodName}, count, 1, TTLocalizer.TIP_KARTING)
                else:
                    loader.beginBulkLoad('hood', TTLocalizer.HeadingToHood % {'to': toHoodPhrase, 'hood': hoodName}, count, 1, TTLocalizer.TIP_GENERAL)
        if hoodId == ToontownGlobals.Tutorial:
    ...
    def enterSEHood(self, requestStatus):
        self.accept(self.hoodDoneEvent, self.handleHoodDone)
        self.hood.enter(requestStatus)
    ...
    def enterCogNationCentral(self, requestStatus):
        self.accept(self.hoodDoneEvent, self.handleHoodDone)
        self.hood.enter(requestStatus)

    def exitCogNationCentral(self):
        self._destroyHood()
    ...
    def getEstateZoneAndGoHome(self, avId, zoneId):
        self.doneStatus = {'avId': avId, 'zoneId': zoneId, 
