...
import TownBattleAttackPanel, TownBattleWaitPanel, TownBattleChooseAvatarPanel, TownBattleSOSPanel, TownBattleSOSPetSearchPanel, TownBattleSOSPetInfoPanel, TownBattleToonPanel
        ...
        if mode == 'Inventory':
            ...
            if self.track == HEAL_TRACK:
                if self.__isGroupHeal(self.level):
                    ...
                    messenger.send(self.battleEvent, [response])
                    self.fsm.request('AttackWait')
                elif config.GetBool('want-mini-server', False):
